...
sequenceDiagram
    ...
    Producer->>Producer: 生成事务ID
    Producer->>RocketMQ: Message
    RocketMQ-->>Producer: result
   
    Producer->>+DataBase: 执行本地事务
    DataBase-->>-Producer: 本地事务的执行结果
    ...
    alt 本地事务执行成功
        ...
        Note right of Consumer: 开始处理业务
        end
        
    else 本地事务执行失败
        rect rgb(255, 240, 240)
        Producer->>RocketMQ: 回滚事务
        Note right of RocketMQ: 丢弃消息
        end
    end
    end
    ...
    alt RocketMQ未收到确认
        ...
        RocketMQ->>Producer: 检查消息是否处理成功
        Producer-->>RocketMQ: 处理结果
        
        alt 成功
            RocketMQ->>Consumer: 消费消息
        else 失败
            Note right of RocketMQ: 等待下次检查
        end
        end
    end
    end
...
所以，RocketMQ 事务消息就类似于二阶段提交 + 定时检查
...
create table rocket_mq_transaction_status
(
...
public interface Sender {
     ...
     * @param message 发送的消息
     * @param consumer 执行业务操作函数
     * @return RocketMQ 的发送结果
     */
    SendReceipt send(String topic, String message, Consumer<String> consumer) throws ClientException;
}
```
...
public class TransactionRocketMqSender implements Sender {
    ...
    @Override
    public SendReceipt send(String topic, String message, Consumer<String> consumer) throws ClientException {
        String transactionId = UUID.randomUUID().toString();
        Message<String> sendMessage = MessageBuilder.withPayload(message).setHeader("transactionId", transactionId).build();
        ...
        // 如果去除当前行代码，RocketMQ 就会调用 UserMessageTransactionChecker 类检查事务是否执行成功
        transaction.commit();
...
public class TestTransactionMessage {
    ...
    @Resource
    private RocketMQTransactionStatusMapper rocketMQTransactionStatusMapper;
    ...
    public void test_send_transaction_message() throws Exception {
        ...
        SendReceipt send = sender.send(topic, prepareToSendMessage, (transactionId) -> {
            try {
                transactionTemplate.executeWithoutResult(transactionStatus -> {
                    // 1. 保存用户信息
                    User user = new User();
                    user.setName("coding-transaction");
                    userMapper.insert(user);
                    // 2. 添加 事务ID 和 业务ID 的绑定关系(用户ID)
                    RocketMQTransactionStatus rocketMQTransactionStatus = new RocketMQTransactionStatus();
                    rocketMQTransactionStatus.setTransactionId(transactionId);
                    rocketMQTransactionStatus.setServiceId(String.valueOf(user.getId()));
                    rocketMQTransactionStatusMapper.insert(rocketMQTransactionStatus);
                });
            } catch (Exception e) {
                log.error("保存用户信息异常", e);
                throw new RuntimeException("本地事务执行异常", e);
            }
        });
        log.info("发送结果为：{}", send);
...
@Slf4j
public class UserMessageTransactionChecker implements RocketMQTransactionChecker {
    ...
    public TransactionResolution check(MessageView messageView) {
        String transactionId = messageView.getProperties().get("transactionId");
        log.info("start execute transaction check: {}", transactionId);
        if (StringUtils.hasLength(transactionId)) {
            RocketMQTransactionStatus rocketMQTransactionStatus = rocketMQTransactionStatusMapper.selectByTransactionId(transactionId);
            if (Objects.isNull(rocketMQTransactionStatus)) {
                return TransactionResolution.ROLLBACK;
            }
            String serviceId = rocketMQTransactionStatus.getServiceId();
            User user = userMapper.selectById(serviceId);
            if (Objects.isNull(user)) {
                return TransactionResolution.ROLLBACK;
            }
            log.info("commit transaction: {}", transactionId);
            return TransactionResolution.COMMIT;
        }
        log.info("rollback transaction: {}", transactionId);
        return TransactionResolution.ROLLBACK;
    }
}
```
